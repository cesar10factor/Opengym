...
import { createState, signState } from '../strava.js';
...
describe('audit log never contains a token, code, or raw state value', () => {
  ...
  // Security review follow-up: GET /api/strava/callback needs no session at all (the signed,
  // single-use `state` stands in for one — see the route's own header comment), so a bad-signature
  // or expired `state` is EXACTLY what an anonymous, unauthenticated `curl` loop can produce
  // without ever having touched /connect. Before this fix that still cost one audit.log entry per
  // attempt; with AUDIT_MAX capped, enough of a loop expires the legitimate old entries out from
  // under it — quietly erasing the trace of an earlier real incident. Not just "no raw state value
  // leaks" (the assertion above) but "no entry at all" — the log's own length is the thing that
  // must not move. Mirrors csrfOk's own refusal a few hundred lines down in server.js, which
  // reasons about the identical shape of problem (reachable pre-auth, one entry per attempt lets
  // anyone fill the log) and lands on the same answer: console.warn, not audit().
  it('an invalid `state` on the callback (bad signature, never issued) adds NOTHING to the audit log, not even a reason code', async () => {
    const before = fs.readFileSync(path.join(dataDir, 'audit.log'), 'utf8');
    const beforeLines = before.split('\n').filter(Boolean).length;

    // Three flavours of "anyone could produce this without a session": a well-formed but
    // never-issued state, garbage, and empty. None of them requires ever having called /connect.
    const neverIssued = signState(secret, createState('nobody-in-particular', Date.now()));
    await fetch(origin + `/api/strava/callback?state=${encodeURIComponent(neverIssued)}`, { redirect: 'manual' });
    await fetch(origin + '/api/strava/callback?state=not-a-real-token', { redirect: 'manual' });
    await fetch(origin + '/api/strava/callback', { redirect: 'manual' });   // no state param at all

    const after = fs.readFileSync(path.join(dataDir, 'audit.log'), 'utf8');
    const afterLines = after.split('\n').filter(Boolean).length;
    assert.equal(afterLines, beforeLines, 'a bad `state` must cost zero audit log entries — it is reachable by anyone, without a session, by construction');
  });
});

...
// This file lives in api/test/ (upstream's convention for the api's test suite — see
// test/server-pairing.test.js), so the server itself is one directory up.
...
// Security review (2026-09-21), point 1: /api/login/verify 403s a disabled account and
// /api/pair/redeem folds it into its generic "invalid or expired code" — /api/link/options had
// neither check. A live code (up to 15 minutes old) for an account an admin just disabled must
// not still be redeemable: the session /api/link/verify would mint is inert while disabled stays
// set, but the passkey it plants in db.creds is permanent and would work again the moment the
// account is re-enabled — so the code has to die with the account, not just go quiet.
describe('a disabled account\'s link code is not redeemable', () => {
  let dataDir, port, origin, proc, secret, uid, code, notFoundBody;

  before(async () => {
    dataDir = fs.mkdtempSync(path.join(os.tmpdir(), 'link-it-disabled-'));
    port = await getFreePort();
    origin = `http://127.0.0.1:${port}`;
    secret = crypto.randomBytes(32).toString('hex');
    fs.writeFileSync(path.join(dataDir, 'secret'), secret, { mode: 0o600 });
    uid = 'u_' + crypto.randomBytes(6).toString('hex');
    code = 'ABCD-2345';
    // The real scenario: the account was enabled when it minted this code (readSession refuses a
    // disabled user outright, so /api/link/code itself is unreachable once disabled — the code
    // has to already exist from before). db.links is seeded directly here for the same reason the
    // other suites in this file seed db.creds directly: no route exists to disable a user's own
    // account onto a pre-existing code in one step, and this is the state that matters, not how
    // it was reached.
    fs.writeFileSync(path.join(dataDir, 'db.json'), JSON.stringify({
      users: [{ id: uid, name: 'Now Disabled', created: new Date().toISOString(), disabled: true }],
      creds: [], subs: [], invites: [],
      links: [{ code, uid, exp: Date.now() + 15 * 60000 }]
    }));
    proc = spawnServer(dataDir, port, origin);
    await waitReady(origin);

    // A baseline not-found response to compare the disabled-account response against byte for
    // byte — the whole point of the fix is that the two must be indistinguishable.
    const nf = await fetch(origin + '/api/link/options', {
      method: 'POST', headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ code: 'ZZZZ-ZZZZ' })
    });
    notFoundBody = await nf.json();
  });
  ...
  it('a live code left over from before the account was disabled is refused with the generic error', async () => {
    const optR = await fetch(origin + '/api/link/options', {
      method: 'POST', headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ code })
    });
    assert.equal(optR.status, 400);
    const body = await optR.json();
    assert.deepEqual(body, notFoundBody, 'a disabled account\'s code must read exactly like a not-found one');
  });
});

// Security review (2026-09-21), point 2: upstream's own /api/logout/all already treats an
// unredeemed pairing code as "a session-in-waiting for this account" and revokes it. A link code
// is strictly more powerful — it plants a permanent passkey, not a session — so "sign out
// everywhere" (exactly what gets pressed after a code may have leaked, or a device with it on
// screen was stolen) has to revoke it too.
/* Single-use survives only while redeeming and planting the credential stay in one synchronous
   run. redeemLink() folds the recheck and the burn together, but nothing stops a later edit from
   dropping an `await` between that call and db.creds.push — and if one lands there, two verifies
   racing on one code both get through and plant two credentials. No behavioural test can catch
   that: reaching this branch over HTTP needs a real WebAuthn ceremony, so the suite would stay
   green. Hence a check on the shape of the code itself. */
describe('link/verify keeps redeem and credential-insert in one synchronous run', () => {
  it('has no await between redeemLink() and db.creds.push()', () => {
    const src = fs.readFileSync(path.join(API_DIR, 'server.js'), 'utf8');
    const start = src.indexOf('redeemLink(db.links');
    assert.notEqual(start, -1, 'redeemLink(db.links...) call not found — did the call site get renamed?');
    const end = src.indexOf('db.creds.push', start);
    assert.notEqual(end, -1, 'no db.creds.push after redeemLink — did the routes get reordered?');

    const between = src.slice(start, end);
    assert.ok(
      !/\bawait\b/.test(between),
      'an `await` appeared between redeemLink() and db.creds.push(). That reopens the double-redeem\n' +
      'race: two /api/link/verify calls on one code would both pass the recheck and both register a\n' +
      'credential. Keep the burn and the insert in the same synchronous run.'
    );
  });
});

describe('POST /api/logout/all revokes a live link code', () => {
  let dataDir, port, origin, proc, secret, uid, cookie, otherUid, otherCookie;

  before(async () => {
    dataDir = fs.mkdtempSync(path.join(os.tmpdir(), 'link-it-logoutall-'));
    port = await getFreePort();
    origin = `http://127.0.0.1:${port}`;
    secret = crypto.randomBytes(32).toString('hex');
    fs.writeFileSync(path.join(dataDir, 'secret'), secret, { mode: 0o600 });
    uid = 'u_' + crypto.randomBytes(6).toString('hex');
    otherUid = 'u_' + crypto.randomBytes(6).toString('hex');
    const cred = id => ({ id: crypto.randomBytes(16).toString('base64url'), userId: id, publicKey: 'AAAA', counter: 0, transports: [], created: new Date().toISOString() });
    fs.writeFileSync(path.join(dataDir, 'db.json'), JSON.stringify({
      users: [
        { id: uid, name: 'Test User', created: new Date().toISOString() },
        { id: otherUid, name: 'Bystander', created: new Date().toISOString() }
      ],
      creds: [
        // logout/all bumps `sv`, which invalidates cookies — but that only matters if the
        // account can still be signed into at all, so it needs at least one credential of its
        // own here (unrelated to the link code under test).
        cred(uid),
        cred(otherUid)
      ],
      subs: [], invites: []
  ...
  it('a link code minted before logout/all is gone from db.links afterwards, and only that user\'s', async () => {
    const codeR = await fetch(origin + '/api/link/code', { method: 'POST', headers: { Cookie: cookie } });
    assert.equal(codeR.status, 200);
    const otherR = await fetch(origin + '/api/link/code', { method: 'POST', headers: { Cookie: otherCookie } });
    assert.equal(otherR.status, 200);
    assert.ok(dbNow().links.some(l => l.uid === uid), 'sanity: the code must actually be persisted first');
    assert.ok(dbNow().links.some(l => l.uid === otherUid), 'sanity: the bystander\'s code must be persisted too');

    const outR = await fetch(origin + '/api/logout/all', { method: 'POST', headers: { Cookie: cookie } });
    assert.equal(outR.status, 200);

    assert.equal(dbNow().links.some(l => l.uid === uid), false, 'logout/all must drop this user\'s link codes');
    // The filter is keyed on uid, so a bug that widened it into "drop every pending code" would
    // sign every other profile's linking out from under them without anyone noticing.
    assert.ok(dbNow().links.some(l => l.uid === otherUid), 'logout/all must not touch another user\'s link code');
  });
});

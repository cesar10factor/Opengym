...
// Security review follow-up: recordStravaUpload's own write (server.js) used to let a disk error
// propagate as an uncaught exception -> 500 -> client retry -> a genuine SECOND upload of a
// workout that had already landed on Strava. The fix mirrors the "unreadable file" branch that
// already existed right next to it: a write failure reads as recorded:false, not a throw. Forcing
// a real disk error portably (Windows and POSIX alike) without touching internals: pre-create the
// atomicWrite tmp path AS A DIRECTORY, so `fs.writeFileSync(tmp, ...)` fails with EISDIR/EPERM
// every time, on every OS — the read of the real (still normal) file beforehand is unaffected.
describe('a write failure while RECORDING an upload never surfaces as a 500', () => {
  let dataDir, port, origin, proc, secret, uid, cookie, stub;
  function stravaFilePath(u) { return path.join(dataDir, 'strava-' + u + '.json'); }
  function uploadsFilePath(u) { return path.join(dataDir, 'strava-uploads-' + u + '.json'); }
  ...
  before(async () => {
    stub = createStravaStub();
    await stub.listen();
    dataDir = fs.mkdtempSync(path.join(os.tmpdir(), 'strava-upload-recordfail-'));
    port = await getFreePort();
    origin = `http://127.0.0.1:${port}`;
    secret = crypto.randomBytes(32).toString('hex');
    fs.writeFileSync(path.join(dataDir, 'secret'), secret, { mode: 0o600 });
    uid = 'u_' + crypto.randomBytes(6).toString('hex');
    seedDb(dataDir, [{ id: uid, name: 'Record Fail User', created: new Date().toISOString() }]);
    cookie = signCookie(secret, uid);
    fs.writeFileSync(stravaFilePath(uid), JSON.stringify({ athleteId: 424242, access: 'A', refresh: 'R', expiresAt: Date.now() + 6 * 60 * 60 * 1000 }));
    // The write this test forces to fail: make atomicWrite's own tmp path an existing directory,
    // so writeFileSync(tmp, ...) throws before renameSync is ever reached. No pre-existing
    // strava-uploads-<uid>.json at all — readStravaUploads sees ENOENT and returns [], exactly the
    // ordinary "first upload ever" case, so this is purely a write-time failure, not a read one.
    fs.mkdirSync(uploadsFilePath(uid) + '.tmp');
    proc = spawnServer(dataDir, port, origin, {
      STRAVA_CLIENT_ID: 'cid', STRAVA_CLIENT_SECRET: 'csecret', STRAVA_API_BASE: stub.base(),
      STRAVA_UPLOAD_POLL_DELAY_MS: '20', STRAVA_TIMEOUT_MS: '300', STRAVA_HIDE_FROM_HOME: '0'
    });
  ...
  it('a successful upload whose RECORD write fails answers 200 with recorded:false, not a 500', async () => {
    stub.setUploadHandler(() => ({ status: 201, body: { id: 6001, external_id: null, status: 'Your activity is still being processed.' } }));
    const r = await fetch(origin + '/api/strava/upload', {
      method: 'POST', headers: { 'Content-Type': 'application/json', Cookie: cookie },
      body: JSON.stringify({ workoutId: 'w-record-fail', payload: samplePayload() })
    });
    assert.equal(r.status, 200, 'the upload itself succeeded — a local write failure must not turn that into an error response');
    const body = await r.json();
    assert.equal(body.recorded, false, 'the record write failed, and the response must say so honestly');
    assert.equal(fs.existsSync(uploadsFilePath(uid)), false, 'the real file must be untouched — only the .tmp directory exists');
  });
});

// Same failure class as above, for queueStravaMute — called from the upload route AFTER the
// upload is already recorded as done, so a write failure here must be even less able to cost the
// workout: muting is cosmetic (hide_from_home is not privacy — see STRAVA_HIDE_FROM_HOME) and the
// brief's own hard rule is that a failed mute is always a 200 with mutePending, never an error.
describe('a write failure while QUEUING a pending mute never surfaces as a 500', () => {
  let dataDir, port, origin, proc, secret, uid, cookie, stub;
  function stravaFilePath(u) { return path.join(dataDir, 'strava-' + u + '.json'); }
  function uploadsFilePath(u) { return path.join(dataDir, 'strava-uploads-' + u + '.json'); }
  function mutesFilePath(u) { return path.join(dataDir, 'strava-mutes-' + u + '.json'); }
  ...
  before(async () => {
    stub = createStravaStub();
    await stub.listen();
    dataDir = fs.mkdtempSync(path.join(os.tmpdir(), 'strava-upload-mutequeuefail-'));
    port = await getFreePort();
    origin = `http://127.0.0.1:${port}`;
    secret = crypto.randomBytes(32).toString('hex');
    fs.writeFileSync(path.join(dataDir, 'secret'), secret, { mode: 0o600 });
    uid = 'u_' + crypto.randomBytes(6).toString('hex');
    seedDb(dataDir, [{ id: uid, name: 'Mute Queue Fail User', created: new Date().toISOString() }]);
    cookie = signCookie(secret, uid);
    fs.writeFileSync(stravaFilePath(uid), JSON.stringify({ athleteId: 424242, access: 'A', refresh: 'R', expiresAt: Date.now() + 6 * 60 * 60 * 1000 }));
    // Same technique as the record-write test above, aimed at the mutes file's tmp path instead —
    // queueStravaMute's own atomicWrite fails every time, deterministically, on every OS.
    fs.mkdirSync(mutesFilePath(uid) + '.tmp');
    proc = spawnServer(dataDir, port, origin, {
      STRAVA_CLIENT_ID: 'cid', STRAVA_CLIENT_SECRET: 'csecret', STRAVA_API_BASE: stub.base(),
      // Muting stays ON (default) — the whole point is exercising queueStravaMute. A large sweep
      // interval keeps the background sweeper from also touching this uid mid-test.
      STRAVA_UPLOAD_POLL_DELAY_MS: '20', STRAVA_TIMEOUT_MS: '300', STRAVA_MUTE_SWEEP_MS: '600000'
    });
    await waitReady(origin);
  });
  ...
  it('an upload that cannot be muted inline, and whose queue write also fails, still answers 200 with mutePending:false', async () => {
    stub.setUploadHandler(() => ({ status: 201, body: { id: 6002, external_id: null, status: 'Your activity is still being processed.' } }));
    // Never resolves an activity_id within the request's own poll budget, so the route falls into
    // the "not settled inline, try to queue" branch every time — the one that calls
    // queueStravaMute, whose write this test has broken.
    stub.setPollHandler(() => ({ status: 200, body: { id: 6002, error: null, status: 'Your activity is still being processed.', activity_id: null } }));

    const r = await fetch(origin + '/api/strava/upload', {
      method: 'POST', headers: { 'Content-Type': 'application/json', Cookie: cookie },
      body: JSON.stringify({ workoutId: 'w-mutequeue-fail', payload: samplePayload() })
    });
    assert.equal(r.status, 200, 'a mute that could not even be QUEUED must never turn an already-recorded upload into an error');
    const body = await r.json();
    assert.equal(body.recorded, true, 'the upload itself was recorded successfully — independent of the mute queue');
    assert.equal(body.muted, false);
    assert.equal(body.mutePending, false, 'the queue write failed, so this must say so rather than falsely promise a later retry');
    assert.equal(fs.existsSync(mutesFilePath(uid)), false, 'the real mutes file must be untouched — only the .tmp directory exists');

    const recorded = JSON.parse(fs.readFileSync(uploadsFilePath(uid), 'utf8'));
    assert.ok(recorded.includes('w-mutequeue-fail'), 'the upload record itself must be unaffected by the mute queue failing');
  });
});

...
export default function Settings() {
  ...
  const linkDeviceHere = async () => {
    try {
      ...
      useUI.getState().openSheet(close => <LinkCodeSheet code={code} exp={exp} close={close} />)
    } catch (e) { toast(e.message || t('Could not generate a code')) }
  }
...
// live countdown to its expiry (server enforces the actual deadline — this is display-only).
function LinkCodeSheet({ code, exp, close }) {
  const [now, setNow] = useState(Date.now())
  useEffect(() => {
    const id = setInterval(() => setNow(Date.now()), 1000)
    return () => clearInterval(id)
  }, [])
  const { minutes, seconds, expired } = remaining(exp, now)
  return <>
    <h3>{t('Link another device')}</h3>
    <div className="muted small" style={{ marginBottom: 14 }}>
      {t('On your other device, tap "Link this device" and enter this code.')}
    </div>
    <div style={{ fontFamily: 'monospace', fontSize: 32, fontWeight: 700, letterSpacing: '.1em', textAlign: 'center', margin: '4px 0 10px' }}>
      {formatCode(code)}
    </div>
    <div className="dim small" style={{ textAlign: 'center', marginBottom: 14 }}>
      {expired ? t('Expired — go back and generate a new code.')
        : t('Expires in {0}:{1}', String(minutes), String(seconds).padStart(2, '0'))}
    </div>
    {/* The code is a key to the account for as long as it lives, so say so plainly here rather
        than only in the docs — this sheet is the one moment the code is on screen and someone
        could photograph it or read it over a shoulder. */}

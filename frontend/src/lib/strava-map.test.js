...
describe('stravaExerciseFor — the test that matters', () => {
  // 1326 = upstream's 1324 plus the fork's two hand-added entries (9001 one-leg calf raise,
  // 9002 paused bench press). The rebase onto upstream v1.3.8 briefly dropped those two and this
  // count was lowered to match, which hid the loss; it is asserted here (not just implied by the
  // loop below) so a future EXDB change is caught explicitly rather than silently changing what
  // "ALL" means.
